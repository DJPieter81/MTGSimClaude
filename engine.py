...
def try_reactive_counter(gs: GameState, caster, defender, spell_card, log_list: list) -> bool:
    ...
    if ctr:
        ...
        gs.strat_log.log_disruption(
            gs.turn, gs, defender, 'counter',
            spell_card.tag or 'spell', _counter_tag,
...
def _strategy_bug(player, opponent, gs, total_mana, log_fn, log_entries):
            ...
            if target_early and budget[0] >= effective_cmc(push_early) and can_afford(player, push_early.mana_cost):
                def _resolve_early_push(c):
                    ...
                    gs.strat_log.log_disruption(
                        ...
                        target_early.card.tag or 'creature', 'push',
            ...
            if snuff_early and has_swamp and not gs.spell_blocked_by_chalice(snuff_early.cmc):
                ...
                if target_early and player.life > CT.SNUFF_LIFE_FLOOR_AGGRO:
                    def _resolve_early_snuff(c):
                        ...
                        gs.strat_log.log_disruption(
                            gs.turn, gs, player, 'remove',
                            target_early.card.tag or 'creature', 'snuff',
    ...
    if ad and budget[0] >= effective_cmc(ad) and can_afford(player, ad.mana_cost):
        ...
        if ad_target:
            ...
            gs.strat_log.log_disruption(
                gs.turn, gs, player, 'remove',
                ad_target.card.tag or 'permanent', 'ad',
        ...
        if has_swamp and snuff_targets and player.life > 4 + 4:  # keep 4 life buffer
            ...
            if target.cmc >= 3 or target.power >= 4 or target.toughness > 3:
                def _resolve_snuff(c):
                    ...
                    gs.strat_log.log_disruption(
                        gs.turn, gs, player, 'remove',
                        target.card.tag or 'creature', 'snuff',
        ...
        if big and budget[0] >= 1 and player.available_mana_count() >= 1:
            if MTGRules.dismember_kills(big):  # L3: only cast if it will kill
                def _resolve_dismember(c):
                    ...
                    gs.strat_log.log_disruption(
                        gs.turn, gs, player, 'remove',
                        big.card.tag or 'creature', 'dismember',
    ...
    if pyro_card and budget[0] >= 1 and can_afford(player, pyro_card.mana_cost):
        ...
        if target_perm:
            def _resolve_pyro(c):
                ...
                gs.strat_log.log_disruption(
                    gs.turn, gs, player, 'remove',
                    target_perm.card.tag or 'creature', c.tag or 'pyro',
    ...
    if bolt and active.creatures and responder.available_mana_count() >= 1:
        ...
        if targets:
            ...
            if _will_die:
                ...
                gs.strat_log.log_disruption(
                    gs.turn, gs, responder, 'remove',
                    target.card.tag or 'creature', 'bolt',
        ...
        if white_pitch:
            ...
            if target and target.power >= 1:
                ...
                def _resolve_sol(c, _t=target, _pitch=white_pitch):
                    ...
                    gs.strat_log.log_disruption(
                        gs.turn, gs, player, 'remove',
                        _t.card.tag or 'creature', 'solitude',
            ...
            def _resolve_crea(c, _tag=tag):
                ...
                if _tag == 'skyclave' and opponent.creatures:
                    ...
                    if target:
                        ...
                        gs.strat_log.log_disruption(
                            gs.turn, gs, player, 'remove',
                            target.card.tag or 'creature', 'skyclave',
...
def _strategy_ocelot(player, opponent, gs, total_mana, log_fn, log_entries):
        ...
        if nonlands:
            def _resolve_ts(c):
                ...
                target = (next((cc for cc in nl if cc.free_cast_if_blue), None)
                          or next((cc for cc in nl if cc.tag in ('daze', 'fow', 'fon')), None)
                          or next((cc for cc in nl if cc.is_removal), None)
                          or next((cc for cc in nl if cc.win_condition or cc.is_combo_piece), None)
                          or next((cc for cc in nl if cc.engine), None)
                          or max(nl, key=lambda cc: cc.cmc))
    ...
    if therapy and opp_can_cast(therapy, budget[0], gs, caster=player) \
        ...
        def _resolve_therapy(c):
            ...
            if nl:
                ...
                name_counts = Counter(cc.name for cc in nl)
                tgt_name, _ = name_counts.most_common(1)[0]
                to_remove = [cc for cc in opponent.hand if cc.name == tgt_name]
                for cc in to_remove:
    ...
    if bombardment and not bombardment_on_board \
            and opp_can_cast(bombardment, budget[0], gs, caster=player) \
            and len(player.creatures) >= 1:
        def _resolve_bomb(c):
            player.put_enchantment_in_play(c)
            log_fn("Goblin Bombardment enters play")
        cast_spell(player, opponent, gs, bombardment, budget, log_fn, log_entries,
                   on_resolve=_resolve_bomb)

    # 5. Deploy creatures
    deploy_priority = ['ocelot', 'guide', 'ajani', 'voice', 'bowm', 'raptor']
    max_deploys = 3 if budget[0] >= 4 else (2 if budget[0] >= 2 else 1)
    deployed = 0
    for tag in deploy_priority:
        ...
        gs.strat_log.log_decision(
            ...
            candidates=deploy_priority,
            chosen=tag,
            reason=f"mana={budget[0]}, deployed={deployed}, "
    ...
    for bolt in bolts:
        ...
        def _resolve_bolt(c, _small=None if go_face else small, _face=go_face):
            ...
            if _face or not _small:
                ...
                gs.strat_log.log_disruption(
                    gs.turn, gs, player, 'remove',
                    _small.card.tag or 'creature', 'bolt',
        ...
        def _resolve_bowm(c):
            ...
            if opponent.creatures:
                ...
                if target.toughness <= 1:
                    ...
                    gs.strat_log.log_disruption(
                        gs.turn, gs, player, 'remove',
                        target.card.tag or 'creature', 'bowm',
    ...
    if snap and can_cast(snap):
        ...
        if fb:
            def _resolve_snap(c, _fb=fb, _stp_fb=stp_fb, _term_fb=term_fb, _bs_fb=bs_fb):
                ...
                if _fb == _stp_fb and opponent.creatures:
