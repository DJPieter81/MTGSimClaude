"""
Sneak & Show (JPA93, 3rd Place) — Legacy Challenge Top 8.
...
- Archon of Cruelty (6/6 flying, drain 3 on ETB): additional payoff.

Fast mana: Ancient Tomb (taps for 2, costs 2 life), City of Traitors (taps for 2),
Lotus Petal (free +1 mana), Simian Spirit Guide (exile from hand for +R).

Cantrips: Brainstorm, Ponder, Stock Up (draw 2 for 2).
Protection: Force of Will, Daze, Flusterstorm.
"""
...
def make_sneak_b_deck():
    d = []

    # -- Lands (18) ------------------------------------------------------------

    # Ancient Tomb x3: taps for 2 colorless, costs 2 life
    for _ in range(3):
    ...
    # City of Traitors x2: taps for 2 colorless (sacrificed when another land enters)
    for _ in range(2):
    ...
    # Simian Spirit Guide x1: exile from hand to add R
    for _ in range(1):
    ...
    # Stock Up x3: instant, draw 2 for 2
    for _ in range(3):
    ...
    # Flusterstorm x2
    for _ in range(2):
        d.append(instant('Flusterstorm', 1, {'U': 1}, {'U'}, tag='fluster'))

    assert len(d) == 60, f"sneak_b deck: {len(d)} cards (expected 60)"
    return d


# --- Strategy -----------------------------------------------------------------

def _strategy_sneak_b(player, opponent, gs, total_mana, log_fn, log_entries):
    """
    Sneak & Show strategy (JPA93, 3rd Place).

    Priority:
    1. Count effective mana (lands + tomb bonus + city bonus + petals + SSG)
    ...
    ssg_in_hand = [c for c in player.hand if c.tag == 'ssg']
    effective_mana = mana + tomb_bonus + city_bonus + len(petals_in_hand) + len(ssg_in_hand)

    # -- Assess hand -----------------------------------------------------------
    payoffs = [c for c in player.hand
               if c.tag in ('emrakul', 'atraxa', 'archon', 'omni', 'sneak')]
    ...
    creature_payoffs = [c for c in player.hand
                        if c.tag in ('emrakul', 'atraxa', 'archon')]

    # -- Step 1: Crack Lotus Petals / exile SSG for mana if we're going to combo
    ...
    if sat and payoffs and mana >= 3 and not gs.game_over:
        if not _try_counter_any(player, opponent, gs, sat, log_entries):
            ...
            # Choose best payoff: Emrakul > Omniscience > Sneak Attack > Archon > Atraxa
            ...
            if best.tag == 'emrakul':
                ...
                perm = player.put_creature_in_play(best)
                log_fn(f"Show and Tell -> Archon of Cruelty (6/6 flying)", True)
                # Archon ETB: opponent loses 3 life, discards, sacrifices;
                # we draw a card, gain 3 life (simplified)
                opponent.life -= 3
                player.life += 3
                player.draw(1)
                ...
                creature_in_hand = next(
                    (c for c in player.hand if c.tag in ('emrakul', 'atraxa', 'archon')),
                    None)
                if creature_in_hand and mana >= 1:
                    ...
                    if creature_in_hand.tag == 'emrakul':
                        ...
                        perm = player.put_creature_in_play(creature_in_hand)
                        perm.summoning_sick = False
                        log_fn(f"Sneak Attack -> Archon (6/6 haste)!", True)
                        opponent.life -= 3
                        player.life += 3
                        player.draw(1)
    ...
    if has_sneak_in_play and not gs.game_over:
        creature_in_hand = next(
            (c for c in player.hand if c.tag in ('emrakul', 'atraxa', 'archon')), None)
        if creature_in_hand and mana >= 1:
...
def test_sneak_b():
    ...
    assert tag_counts['archon'] == 1, f"Archon count: {tag_counts['archon']}"
    assert tag_counts['omni'] == 1, f"Omniscience count: {tag_counts['omni']}"
    assert tag_counts['fow'] == 4, f"FoW count: {tag_counts['fow']}"
    assert tag_counts['petal'] == 4, f"Petal count: {tag_counts['petal']}"
    assert tag_counts['fluster'] == 2, f"Flusterstorm count: {tag_counts['fluster']}"
    assert tag_counts['ssg'] == 1, f"SSG count: {tag_counts['ssg']}"
    assert tag_counts['daze'] == 2, f"Daze count: {tag_counts['daze']}"
    ...
    # Test 5: Mana sources
    assert tag_counts.get('tomb', 0) == 3, "Ancient Tomb x3"
    assert tag_counts.get('city', 0) == 2, "City of Traitors x2"
    results.append("ok Sol lands correct")

    # Test 6: Bo3 smoke test
    from sim import STRATEGIES
    from cards import DECKS
    DECKS['sneak_b'] = make_sneak_b_deck
    STRATEGIES['sneak_b'] = _strategy_sneak_b
    try:
        from sim import run_any_bo3
        r = run_any_bo3('sneak_b', 'dimir', 10)
        results.append(f"ok Sneak B vs Dimir (10 matches): {r['match_wr']*100:.0f}%")
...
if __name__ == '__main__':
    print("Running Sneak B tests...")
    for r in test_sneak_b():
        print(f"  {r}")


# ─── Deck Metadata (auto-registration) ──────────────────────────────────────

def _keep_sneak_b(hand, matchup=''):
    lands = [c for c in hand if c.is_land()]
    nonlands = [c for c in hand if not c.is_land()]
    lc = len(lands)
    tags = {c.tag for c in hand}
    has_sat = 'sat' in tags
    has_payoff = any(t in tags for t in ('emrakul', 'atraxa', 'omni', 'sneak', 'archon'))
    has_cantrip = any(c.is_cantrip for c in nonlands)
    fast_mana = sum(1 for c in hand if c.tag in ('petal', 'tomb', 'city', 'ssg'))
    mana_ok = lc >= 1 or fast_mana >= 1
    has_action = has_sat or has_payoff or has_cantrip
    if len(hand) <= 5: return mana_ok and has_action
    return mana_ok and 1 <= lc <= 5 and has_action


DECK_META = {
    'key':        'sneak_b',
    'name':       'Sneak & Show B (JPA93)',
    'make_deck':  make_sneak_b_deck,
