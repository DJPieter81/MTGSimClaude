...
29. **Combo deck audit produced six independent classes of bug** *(2026-05-03, PR #111, full writeup `docs/lessons/2026-05-03_combo_deck_audit.md`)*. Tracing the lowest-WR combo matchups (doomsday vs ur_delver 12.5%, reanimator vs burn 20%, depths vs burn 35%) found seven bugs — one card-data error (DD CMC=5 instead of BBB=3), two tier-1 omissions (Lion's Eye Diamond missing from DD, Lotus Petal missing from ANT), one off-by-one in a deck-name gate (combo-land priority hook hardcoded to `'lands'`, missed `'depths'`), one strategy/preamble interaction (Reanimator's T2 ritual mana eaten by shared `_execute_turn` Thoughtseize), one heuristic over-counting (Eidolon post-strategy treated cycled cards as missed casts), and one rule violation (Oracle ETB win used `≤` not `<`). Single-line fixes lifted depths vs burn 35→62% (+27pp) and storm vs dnt 34→50% (+16pp). **For Manu**: every Modern combo deck almost certainly has at least one of these bug classes. The diagnostic workflow is mechanical: rank matchups by `|sim_wr − expected_wr|`, generate 5-10 deep traces of the worst, read every line, then break down conditional WR by turn — discontinuities in the cast-turn → win-rate curve point at the bug. The bug taxonomy (cards.py / deck-construction / strategy-preamble / off-by-one-gate / heuristic / rule) is closed under Magic; future audits should classify each finding into one of those five buckets.
    ...
    **Audit checklist for any combo deck** (Modern + Legacy):
    ...
    7. **Fetch validity (round-3 addition):** for every fetch land in a deck, count basics with matching subtypes. If zero, the fetch is pure life-loss — replace it with a basic that matches its targets. Legacy hit this on Eldrazi: 4 Abundant Countryside fetching `{Forest, Plains, Island, Swamp, Mountain}` but the deck had no basics → +9.5pp on `eldrazi vs burn` after replacing with 4 basic Wastes.
    8. **Bo1 hate-card density (round-3 addition):** for each combo / control deck, check whether the matchup-specific hate card is at the count needed for a Bo1 sample. A 2-of card has ~22% chance to be in opener; if the matchup hinges on that card being seen, **3-of is the minimum for the sim to evaluate the matchup faithfully**. Real Bo3 lists run 1-2 main + sideboard, but the sim is Bo1 and needs higher main counts. Legacy hit this on Wan Shi Tong: bumping Sanctifier en-Vec 2→3 lifted `wst vs burn` 30.5→40.5%.

30. **When a "structural" gap is actually still Class B** *(2026-05-03 round-3 follow-up, PR #113)*. The round-1 audit deferred two matchups as "fundamental sim-architecture gaps": `painter vs eldrazi 30%` and `wan_shi_tong 32%`. Round 3 re-audited both and found they were **deck-construction bugs after all** — not architectural. Eldrazi's Countryside without basics, WST's underpowered Sanctifier count. Lesson: before declaring a gap "architectural," do a thorough Class A + B audit with the new lessons #29 checklist questions 7 and 8 above. The real architectural-only gap turned out to be `doomsday vs burn 8-10%` — re-confirmed as the canonical tier-1 list (4 LED, 4 Petal, 4 Dark Ritual, 4 Wraith, 1 Lurrus matches mtgtop8). Remaining gap requires Cabal Therapy modeling + per-matchup pile construction (vs Burn = lifegain pile, vs Control = protection pile). Multi-day project, separate workstream.

31. **Build a `KNOWN[card] → (cmc, mana_cost)` reference table and grep-walk all decks** *(2026-05-04, PR #115)*. The round-5 systematic Class A audit found **18 CMC mismatches** that prior trace-driven audits had missed — most prevalent: Daze at CMC=2 in 12 decks (real CMC=1, the `{1}` in Daze's text is the *opponent's* tax to avoid the counter, not the caster's cost), and Sneak Attack at CMC=4 in 3 decks (real CMC=3). The bug pattern repeats because card-data lives in many deck builders that all hand-encode the same card; one misread propagates by copy-paste. **For Manu**: dump a JSON of `(card_name, cmc, mana_cost)` from a Scryfall snapshot, walk every deck, diff against actual sim values. The grep is mechanical and finds bugs that no number of trace audits will surface — the strategy uses the wrong cmc gate but still "works" because cast_spell doesn't validate mana. Found one nasty test that *baked in the bug* by asserting Daze.cmc == 2; the test name even hedged ("Daze CMC=2 (not 1)") which should be a red flag during code review. Same pattern likely in Modern.

32. **When a strategy's cantrip flow is too coupled to refactor, accept the limitation and document it** *(2026-05-04 PR #115)*. Tried to apply the cephalid-style cantrip resolve_cantrip fix to TES.  TES's Brainstorm putback is tightly coupled to its combo-flow ordering — the player manually picks what to put back based on whether they're digging for combo or pre-emptively pitching unwanted cards, and the generic `resolve_cantrip` keep-val heuristic can't model that.  Generic fix regressed `tes vs dimir` 38.5 → 31% at n=500.  **Reverted with documentation** rather than papered over. Lesson: not every "deck-local code drifted from engine" finding can be cleaned up by routing through the engine helper.  Some strategies have legitimate per-deck logic that the engine's generic helpers don't capture.  Mark the limitation explicitly so future audits don't re-trip on it.

33. **The bug-class taxonomy is closed but skewed** *(2026-05-04 7-round summary)*.  Across 7 audit rounds and 30+ bugs, every finding fit the 6-class taxonomy without needing new classes.  The distribution is heavily skewed toward Class A (card data, 17 bugs) + Class B (deck construction, 11 bugs).  Engine-level bugs (C-F) are rarer (~4 total) but each has high blast radius (the Eidolon false-positive affected every cycling cast in every game; the Oracle `<` vs `≤` affected every DD turn).  **For Manu's audit:** budget time accordingly — most bugs will be deck-data nits (cheap to fix, modest WR impact each); reserve a smaller pass for engine-level pattern matching with the higher-impact targets.

34. **Shared-preamble class of bugs in `sim._execute_turn`** *(2026-05-16, PRs #165 + #166 + #167)*. The coordination layer (`sim._execute_turn`) made tactical decisions for the active player BEFORE the per-deck strategy ran, and per-deck strategies operated on the SAME resources. Three structural fixes in one session showed this is a class:
    - **PR #165 (TS preamble).** `sim._execute_turn:687-743` cast Thoughtseize for ANY deck with `tag='ts'` before strategy dispatch. Mardu's T1 Grief+Ephemerate engine requires a non-Grief black card in hand as evoke pitch fuel; the shared preamble consumed Thoughtseize (the only black non-Grief card in typical openers), so the strategy's `if blacks: ...` gate failed silently. Fix: delete the preamble, migrate TS branches into 4 decks that lacked their own. Mardu vs bug 10% → 19.5%.
    - **PR #166 (Lotus Petal preamble).** `sim._execute_turn:633` added `total_mana += sum(petals in hand)` BEFORE strategy dispatch. Every petal-using strategy ALSO cracked petals with `total_mana += 1` — every Petal in opener counted twice. Painter T1 with 0 lands + 1 Petal deployed 4 nonland artifacts (phantom 3 mana). Fix: framework counts lands only, strategies own all fast mana. Storm corrected −10pp vs Dimir (had been inflated 70%, now 60%).
    - **PR #167 (Belcher LED in response to activation).** Not a preamble per se but the same class — strategy missed an interaction-stack-priority action. Belcher's `_strategy_belcher` only cracked LED inside the Burning Wish branch; the canonical real-Legacy line "cast Charbelcher, crack LED in response to activation cost (CR 605.1)" never fired. Fix: extend the Belcher cast block to crack LEDs when post-LED mana would cover activation. Belcher avg WR 26% → 40.4%.

    **Diagnostic pattern.** Each bug was found by the same workflow: identify under-performing deck → replay one losing seed with `trace=True` → compare what the strategy did to what real Magic would do → identify the divergence point → trace back to find the shared-resource race.

    **What I checked and ruled out.** A/B test (disable framework auto-Tomb-tap, disable framework auto-Wasteland) across 5 decks × 50 seeds: **0pp WR shift** on both. Tomb auto-tap and Wasteland auto-activation are architectural smell but not measurable bugs in the current sim. Per CLAUDE.md's "no fix without failing test", stopped pulling the thread.

    **For Manu**: `sim._execute_turn` (or its Modern equivalent) is the canonical bug-surface for this pattern. Audit any shared resource manipulation that the per-deck strategy might also touch — especially TS-like discard, Petal-like fast mana, and mana-ability-in-response-to-activation. The fix template is "delete the preamble, give each strategy its own branch". Failing-test-first names the mechanic, not the deck.
