# Ticket: migrate `deck_construction` into `tests/rules/test_deck_construction.py`

**Source range**: `sim.py:2157-2166` inside `run_rules_tests()`
**Tests to migrate**: 3
**Target file**: `tests/rules/test_deck_construction.py`
...
- `tests/rules/test_deck_construction.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_construction.py -n auto` is green.
2. Test count matches: there are exactly **3** `def test_…` functions in
...
1. Open `sim.py` lines 2157-2166. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_construction (3 asserts) [ticket]`
