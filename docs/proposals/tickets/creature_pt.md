# Ticket: migrate `creature_pt` into `tests/rules/test_creature_pt.py`

**Source range**: `sim.py:1953-1954` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_creature_pt.py`
...
- `tests/rules/test_creature_pt.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_creature_pt.py -n auto` is green.
...
1. Open `sim.py` lines 1953-1954. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate creature_pt (2 asserts) [ticket]`
