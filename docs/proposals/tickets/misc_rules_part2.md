# Ticket: migrate `misc_rules_part2` into `tests/rules/test_misc_rules_part2.py`

**Source range**: `sim.py:2391-2682` inside `run_rules_tests()`
**Tests to migrate**: 15
**Target file**: `tests/rules/test_misc_rules_part2.py`
...
- `tests/rules/test_misc_rules_part2.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_misc_rules_part2.py -n auto` is green.
...
1. Open `sim.py` lines 2391-2682. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate misc_rules_part2 (15 asserts) [ticket]`
