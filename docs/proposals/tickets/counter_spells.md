# Ticket: migrate `counter_spells` into `tests/rules/test_counter_spells.py`

**Source range**: `sim.py:4555-4699` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_counter_spells.py`
...
- `tests/rules/test_counter_spells.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_counter_spells.py -n auto` is green.
...
1. Open `sim.py` lines 4555-4699. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate counter_spells (2 asserts) [ticket]`
