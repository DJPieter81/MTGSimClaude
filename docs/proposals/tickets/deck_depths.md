# Ticket: migrate `deck_depths` into `tests/rules/test_deck_depths.py`

**Source range**: `sim.py:3700-3704` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_deck_depths.py`
...
- `tests/rules/test_deck_depths.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_depths.py -n auto` is green.
...
1. Open `sim.py` lines 3700-3704. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_depths (2 asserts) [ticket]`
