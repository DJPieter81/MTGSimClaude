# Ticket: migrate `lands_mechanics` into `tests/rules/test_lands_mechanics.py`

**Source range**: `sim.py:1980-4539` inside `run_rules_tests()`
**Tests to migrate**: 16
**Target file**: `tests/rules/test_lands_mechanics.py`
...
- `tests/rules/test_lands_mechanics.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_lands_mechanics.py -n auto` is green.
2. Test count matches: there are exactly **16** `def test_…` functions in
...
1. Open `sim.py` lines 1980-4539. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate lands_mechanics (16 asserts) [ticket]`
