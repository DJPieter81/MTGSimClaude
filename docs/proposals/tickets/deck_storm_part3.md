# Ticket: migrate `deck_storm_part3` into `tests/rules/test_deck_storm_part3.py`

**Source range**: `sim.py:4841-4841` inside `run_rules_tests()`
**Tests to migrate**: 1
**Target file**: `tests/rules/test_deck_storm_part3.py`
...
- `tests/rules/test_deck_storm_part3.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_storm_part3.py -n auto` is green.
...
1. Open `sim.py` lines 4841-4841. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_storm_part3 (1 asserts) [ticket]`
