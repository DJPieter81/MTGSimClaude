# Ticket: migrate `stack_priority` into `tests/rules/test_stack_priority.py`

**Source range**: `sim.py:2295-2295` inside `run_rules_tests()`
**Tests to migrate**: 1
**Target file**: `tests/rules/test_stack_priority.py`
...
- `tests/rules/test_stack_priority.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_stack_priority.py -n auto` is green.
...
1. Open `sim.py` lines 2295-2295. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate stack_priority (1 asserts) [ticket]`
