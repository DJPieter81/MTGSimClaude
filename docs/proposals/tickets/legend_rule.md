# Ticket: migrate `legend_rule` into `tests/rules/test_legend_rule.py`

**Source range**: `sim.py:2267-2268` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_legend_rule.py`
...
- `tests/rules/test_legend_rule.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_legend_rule.py -n auto` is green.
...
1. Open `sim.py` lines 2267-2268. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate legend_rule (2 asserts) [ticket]`
