# Ticket: migrate `mana_pool` into `tests/rules/test_mana_pool.py`

**Source range**: `sim.py:2017-3501` inside `run_rules_tests()`
**Tests to migrate**: 16
**Target file**: `tests/rules/test_mana_pool.py`
...
- `tests/rules/test_mana_pool.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_mana_pool.py -n auto` is green.
...
1. Open `sim.py` lines 2017-3501. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate mana_pool (16 asserts) [ticket]`
