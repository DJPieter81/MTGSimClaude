# Ticket: migrate `cr_113` into `tests/rules/test_cr_113.py`

**Source range**: `sim.py:1919-1924` inside `run_rules_tests()`
**Tests to migrate**: 6
**Target file**: `tests/rules/test_cr_113.py`
...
- `tests/rules/test_cr_113.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_cr_113.py -n auto` is green.
2. Test count matches: there are exactly **6** `def test_…` functions in
...
1. Open `sim.py` lines 1919-1924. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate cr_113 (6 asserts) [ticket]`
