# Ticket: migrate `deck_lands` into `tests/rules/test_deck_lands.py`

**Source range**: `sim.py:2604-2829` inside `run_rules_tests()`
**Tests to migrate**: 8
**Target file**: `tests/rules/test_deck_lands.py`
...
- `tests/rules/test_deck_lands.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_lands.py -n auto` is green.
2. Test count matches: there are exactly **8** `def test_…` functions in
...
1. Open `sim.py` lines 2604-2829. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_lands (8 asserts) [ticket]`
