# Ticket: migrate `force_of_will` into `tests/rules/test_force_of_will.py`

**Source range**: `sim.py:2079-4746` inside `run_rules_tests()`
**Tests to migrate**: 7
**Target file**: `tests/rules/test_force_of_will.py`
...
- `tests/rules/test_force_of_will.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_force_of_will.py -n auto` is green.
2. Test count matches: there are exactly **7** `def test_…` functions in
...
1. Open `sim.py` lines 2079-4746. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate force_of_will (7 asserts) [ticket]`
