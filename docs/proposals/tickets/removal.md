# Ticket: migrate `removal` into `tests/rules/test_removal.py`

**Source range**: `sim.py:1937-3990` inside `run_rules_tests()`
**Tests to migrate**: 21
**Target file**: `tests/rules/test_removal.py`
...
- `tests/rules/test_removal.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_removal.py -n auto` is green.
2. Test count matches: there are exactly **21** `def test_…` functions in
...
1. Open `sim.py` lines 1937-3990. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate removal (21 asserts) [ticket]`
