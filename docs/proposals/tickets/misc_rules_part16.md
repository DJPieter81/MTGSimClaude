# Ticket: migrate `misc_rules_part16` into `tests/rules/test_misc_rules_part16.py`

**Source range**: `sim.py:4850-4867` inside `run_rules_tests()`
**Tests to migrate**: 5
**Target file**: `tests/rules/test_misc_rules_part16.py`
...
- `tests/rules/test_misc_rules_part16.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_misc_rules_part16.py -n auto` is green.
2. Test count matches: there are exactly **5** `def test_…` functions in
...
1. Open `sim.py` lines 4850-4867. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate misc_rules_part16 (5 asserts) [ticket]`
