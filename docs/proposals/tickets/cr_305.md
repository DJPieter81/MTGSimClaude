# Ticket: migrate `cr_305` into `tests/rules/test_cr_305.py`

**Source range**: `sim.py:2063-2066` inside `run_rules_tests()`
**Tests to migrate**: 4
**Target file**: `tests/rules/test_cr_305.py`
...
- `tests/rules/test_cr_305.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_cr_305.py -n auto` is green.
2. Test count matches: there are exactly **4** `def test_…` functions in
...
1. Open `sim.py` lines 2063-2066. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate cr_305 (4 asserts) [ticket]`
