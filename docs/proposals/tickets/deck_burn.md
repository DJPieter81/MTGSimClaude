# Ticket: migrate `deck_burn` into `tests/rules/test_deck_burn.py`

**Source range**: `sim.py:2647-3956` inside `run_rules_tests()`
**Tests to migrate**: 3
**Target file**: `tests/rules/test_deck_burn.py`
...
- `tests/rules/test_deck_burn.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_burn.py -n auto` is green.
...
1. Open `sim.py` lines 2647-3956. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_burn (3 asserts) [ticket]`
