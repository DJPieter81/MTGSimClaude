# Ticket: migrate `deck_reanimator` into `tests/rules/test_deck_reanimator.py`

**Source range**: `sim.py:3492-3495` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_deck_reanimator.py`
...
- `tests/rules/test_deck_reanimator.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_deck_reanimator.py -n auto` is green.
2. Test count matches: there are exactly **2** `def test_…` functions in
...
1. Open `sim.py` lines 3492-3495. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate deck_reanimator (2 asserts) [ticket]`
