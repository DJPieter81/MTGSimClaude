# Ticket: migrate `free_counters` into `tests/rules/test_free_counters.py`

**Source range**: `sim.py:2098-2099` inside `run_rules_tests()`
**Tests to migrate**: 2
**Target file**: `tests/rules/test_free_counters.py`
...
- `tests/rules/test_free_counters.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_free_counters.py -n auto` is green.
...
1. Open `sim.py` lines 2098-2099. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate free_counters (2 asserts) [ticket]`
