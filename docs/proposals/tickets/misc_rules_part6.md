# Ticket: migrate `misc_rules_part6` into `tests/rules/test_misc_rules_part6.py`

**Source range**: `sim.py:3232-3296` inside `run_rules_tests()`
**Tests to migrate**: 15
**Target file**: `tests/rules/test_misc_rules_part6.py`
...
- `tests/rules/test_misc_rules_part6.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_misc_rules_part6.py -n auto` is green.
...
1. Open `sim.py` lines 3232-3296. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate misc_rules_part6 (15 asserts) [ticket]`
