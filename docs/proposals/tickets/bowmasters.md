# Ticket: migrate `bowmasters` into `tests/rules/test_bowmasters.py`

**Source range**: `sim.py:1976-2278` inside `run_rules_tests()`
**Tests to migrate**: 5
**Target file**: `tests/rules/test_bowmasters.py`
...
- `tests/rules/test_bowmasters.py` (rewrite this stub completely)
...
1. `python3 -m pytest tests/rules/test_bowmasters.py -n auto` is green.
...
1. Open `sim.py` lines 1976-2278. Look at the `test(name, actual, expected, detail)`
   pattern.
2. For each call:
...
Commit message: `tests: migrate bowmasters (5 asserts) [ticket]`
