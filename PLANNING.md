...
| Date | Key Changes |
|---|---|
| May 17 2026 (PR #232) | **Audit-driven structural fixes — 13 commits closing 10 partial audits.** Introduces `engine.select_pitch_target(hand, color, exclude, protected_tags)` — generic helper for every "exile a card of color X as alternative cost" mechanic (FoW/FoN, Grief/Fury/Solitude/Endurance evoke, Chrome Mox imprint, Force of Vigor). Wires the helper at 7 callsites across engine.py + decks/{goblins,belcher}.py. Filters via `is_combo_piece` / `win_condition` flags + deck-specific protected-tag set. Other fixes: painter COMBO_META + combo_plan Defer gating (closes 2 audits); painter Portable Hole / Kozilek's Command deployment branch; painter Chalice-aware Karn-over-Ring priority; goblins Vial-flash ETB triggers via shared `_fire_goblin_etb` helper (CR 603.6a); goblins decklist +2 Wasteland; goblins Ringleader-before-Warchief deploy order; WST Chalice@1 library + matchup-aware gate; WST decklist Wasteland 1→3; WST Karakas legendary-bounce + `LEGENDARY_CREATURE_TAGS` extraction in game.py; belcher Wish gate requires post-resolve mana for fetched Empty (3 with LED, 6 without). 24 new @fast tests (495→519). Sweep deltas (n=200): painter vs sneak_b +7.5pp, painter vs burn +3pp, mardu vs ur_tempo +6.5pp, goblins vs ur_tempo +9.5pp, belcher vs ur_delver +9pp, WST vs mono_black +6.5pp, WST vs dnt +5pp, WST vs cloudpost +4pp, goblins vs depths +3.5pp. Regression sweep + abstraction-contract clean. |
| May 04 2026 (PR #115) | **Audit rounds 4-5 — Belcher/TES tier-1 + systematic Class A.** Round 4: Belcher Probe 3→4 + Tinder Wall 2→4, TES Ponder 2→4 (small Class B cleanups). Round 5: built `KNOWN[card]→(cmc, cost)` reference table, grep-walked all 36 decks, found **18 CMC mismatches**. Two high-impact: **Daze CMC=2 in 12 decks** (real CMC=1; the `{1}` is the opponent's tax not the caster's cost) and **Sneak Attack CMC=4 in 3 decks** (real CMC=3). Plus cantrip resolve_cantrip pattern repeated in dimir_c + dimir_d. Sweep deltas: `dimir_c vs burn` 17.5→20%, `dimir_c vs storm` 39.5→43%, `dimir_d vs burn` 14.5→19%, `sneak_a vs bug` 39.5→45%. TES Brainstorm fix attempted then reverted (tight combo-flow coupling; flagged as known limitation). Tests 161→165. Lessons updated with new audit step: dump JSON Scryfall snapshot, grep-diff all decks. |
| May 03 2026 (PR #113) | **Structural-gap closure — Eldrazi basics + WST Sanctifier.** Round-3 audit re-examined the two gaps deferred from PR #111 ("painter vs eldrazi 30%" and "wan_shi_tong 32%") and found both were Class B (deck-construction) bugs, not architectural. **Eldrazi:** ran 4 Abundant Countryside but had zero basics → fetches did nothing, lost 1 life each → silently nerfed by 4 mana sources. Replaced with 4 basic Wastes. `eldrazi vs burn` 39→48.5%. **Wan Shi Tong:** Sanctifier en-Vec at 2 copies (~22% opener vs Burn) → bumped to 3 (canonical Bo1 ratio). `wst vs burn` 30.5→40.5%. Doomsday vs Burn re-audited and confirmed truly architectural (needs Cabal Therapy + per-matchup pile construction). Tests 158→160. Lessons updated with two new audit-checklist questions: fetch validity (basics matching fetch targets) + Bo1 hate-card density (3-of minimum for matchup-pivotal cards). |
| May 03 2026 (PR #112) | **Audit round 2 — cephalid Brainstorm + wizardcycling.** Class A bugs (deck-local code drifted from engine): cephalid's local cantrip handler did `player.draw(1)` for both Brainstorm and Ponder (Brainstorm should draw 3, put 2 back); Step Through gated on full {1}{U}{U} cast cost instead of {U} wizardcycling activation. `cephalid vs dimir` 31.7→36.5%, `cephalid vs lands` 60%. Tests 157→158. Audit checklist gained Class-A grep one-liner: `grep -rn "draw(1)" decks/*.py` — every hit should be a non-Brainstorm cantrip. |
