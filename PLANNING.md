...
1. **Finish skipped `cast_spell()` conversions** — close the Eidolon-coverage gap left by Phase B SKIPs. _strategy_doomsday, _strategy_oops, _strategy_storm, _strategy_show, _strategy_mardu were converted 2026-04-19. _strategy_bug (21 casts; Abrupt Decay intentionally left manual since it is officially uncounterable) converted 2026-04-23. **Remaining: `decks/belcher` (16 casts) and `decks/tes` (33 casts)** — both define a local `cast_spell` helper that shadows `engine.cast_spell`; rename the local helper before converting. Pattern documented in `docs/ARCHIVED_SESSION_TASK_2026-04-13.md`.
